// Hero plasma background — vanilla port of the 21st.dev "Plasma" shader
// component (light blue plasma w/ grain). The React wrapper is replaced by a
// plain IIFE that mounts a WebGL canvas into #hero-bg; the shader source and
// every tuning value are identical to the original component.
//
// The div itself carries the 20% opacity, so the shader renders at full
// brightness and the fade is uniform, grain included.
//
// Renders on demand: paused when the tab is hidden or the hero is scrolled
// out of view, and prefers-reduced-motion gets a single static frame.
(function () {
    'use strict';
    ...
    // Verbatim from the original component ("light blue plasma w/ grain").
    var UNIFORMS = {
        colors: [
    ...
    gl.uniform4f(uni.cursor, 0, 4.0, 0.65, 0.297);
    ...
    // Reduced motion still gets the plasma, just frozen: one frame, no loop.
    ...
    function resizeCanvas() {
        ...
        var rawW = Math.max(1, Math.round(bounds.width * dpr));
        var rawH = Math.max(1, Math.round(bounds.height * dpr));
    ...
    if ('ResizeObserver' in window) {
        new ResizeObserver(function () { requestRender(); }).observe(canvas);
    } else {
