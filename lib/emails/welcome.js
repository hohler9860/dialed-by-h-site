...
// ── Color tokens ─────────────────────────────────────────────
// Dark by design: the whole email is the same #111 the "How It Works" section
// uses, with light text baked in inline. There is no light variant and no
// prefers-color-scheme dependence, so every client (Gmail apps included)
// shows the same thing — client dark-mode transforms target light emails and
// leave already-dark ones alone. Still NO background/text "locks" (no
// same-color gradients, no -webkit-text-fill-color): locks are what caused
// the unreadable-mush bug when a client does decide to transform.
const C = {
  bg: "#050505",
  card: "#111111",
  cardBorder: "#2a2a2a",
...
// ── Shared CSS ───────────────────────────────────────────────
// The email is dark by design (see palette C), so there is no dark-mode media
// query anymore — the inline colors ARE the only version. This block adds an
// opacity:1 safety net so entrance animations can never strand text invisible
// at their opacity:0 keyframe start, plus mobile sizing.
const SHARED_STYLES = `
...
function emailHead(previewText) {
  return [
    React.createElement(
      ...
      // No color-scheme metas: the email ships one dark version only, and
      // declaring "light dark" would invite clients to re-theme it.
    ...
    React.createElement(
      ...
      React.createElement("tr", null,
        React.createElement("td", {
          ...
          className: "email-bg",
          style: { backgroundColor: C.bg, padding: "32px 12px" },
        },
          React.createElement(
            "table", {
              ...
              cellPadding: "0", cellSpacing: "0", border: "0",
              className: "email-bg",
              // bordered card on a darker page bg, so the email reads as a
              // clearly bounded object instead of one endless dark field
              style: { backgroundColor: C.card, border: `1px solid ${C.cardBorder}`, borderCollapse: "separate", borderSpacing: "0", maxWidth: "600px" },
            },
            ...children
          )
        )
...
function logoRow() {
  // Bare white wordmark on transparent — no plaque box. The plaque only
  // existed to survive light/dark inversion, moot now the email is dark-only.
  return row(C.card, "0 48px 34px", "center",
    React.createElement(Link, { href: BRAND.siteUrl, style: { textDecoration: "none" } },
      React.createElement(Img, {
        src: BRAND.logo, alt: "Dialed By H", width: "220",
        style: { display: "block", width: "220px", height: "auto", outline: "none", border: "none" },
      })
    )
  );
}
...
// stickerSrc pins a specific sticker; otherwise one is picked from the seed.
function headerBlock(tag, headline, seed, stickerSrc) {
  const sticker = stickerSrc || pickSticker(seed);
  return React.createElement(
    "table", {
...
// full-width section panel: slightly lighter block with crisp 1px edges, the
// same treatment the old light design gave the dark "How It Works" block
function panelSection(children, padding) {
  return React.createElement("tr", null,
    React.createElement("td", {
      bgcolor: C.panel, className: "content-pad",
      style: {
        backgroundColor: C.panel,
        borderTop: `1px solid ${C.panelBorder}`,
        borderBottom: `1px solid ${C.panelBorder}`,
        padding: padding || "36px 48px",
      },
    }, children)
  );
}

// mono label used at the top of a panelSection
function panelLabel(text) {
  return React.createElement(Text, {
    key: "panel-label",
    className: "muted-text",
    style: { color: C.muted, fontSize: "10px", fontWeight: "700", fontFamily: FONT.mono, letterSpacing: "4px", textTransform: "uppercase", margin: "0 0 24px" },
  }, text);
}
...
module.exports = {
  ...
  fullBleedImage, trustRow, citiesStrip, panelSection, panelLabel,
};
