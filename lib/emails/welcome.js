...
// DARK BY DESIGN. This is the only reliably-readable option: the Gmail mobile
// apps cannot be forced to keep a light email light in dark mode — they lighten
// the text while the (locked) background stays white, producing unreadable
// grey-on-white. A genuinely dark email sidesteps that entirely: dark-mode
// clients leave already-dark emails alone, and light inboxes show it as an
// intentional premium dark card. Same look everywhere, always readable.
const C = {
  bg: "#151515",
  card: "#151515",
  heading: "#f4f4f4",
...
// ── Shared CSS ───────────────────────────────────────────────
// The palette (C) is dark by design, so there is no dark-mode fight to win here.
// The one safety net below forces opacity:1 on the animated elements: entrance
// animations play where supported, but if a client freezes or ignores them the
// text can never get stuck invisible at the keyframe's opacity:0 start.
const SHARED_STYLES = `
