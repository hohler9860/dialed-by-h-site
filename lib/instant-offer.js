...
const { guard: rateGuard, getClientIp } = require("./ratelimit.js");
const security = require("./offer-security.js");
const crypto = require("crypto");
const { fetchAllPieces } = require('../api/_pieces');
const { searchCatalog, lookupCatalogMarkets } = require('./catalog-search');
const guard = options => rateGuard({ ...options, failClosed: true });
...
const configuredMargin = Number(process.env.INSTANT_OFFER_MARGIN || 0.15);
const MARGIN = Number.isFinite(configuredMargin) ? Math.min(0.4, Math.max(0.05, configuredMargin)) : 0.15;
...
function priceIt(market, { condition, set }) {
  if (market && (!Number.isSafeInteger(Number(market.n)) || Number(market.n) < 0 ||
      ![market.median, market.p10, market.p90].every(v => v !== null && v !== '' && Number.isFinite(Number(v)) && Number(v) > 0) ||
      Number(market.p10) > Number(market.median) || Number(market.median) > Number(market.p90))) {
    return { ok: false, reason: 'invalid_market', market: null };
  }
...
function offerEmail(row) {
  const rows = [
    ...
    ["Notes", row.notes],
    ["Photos", (row.photos || []).length ? `${row.photos.length} stored securely with submission ${row.id}` : "none"],
  ...
  return `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;color:#1a1a1a;background:#fafafa">
      ...
      <h2 style="font-size:18px;font-weight:700;margin:0 0 20px">${row.sell_option === "consign" ? "Consignment request" : row.offer_response?.mode === "personal" ? "Personal quote request" : row.needs_review ? "Instant Offer needs your OK" : "Instant Offer received"}</h2>
...
async function handle(req, res) {
  ...
  if (body.website || body.company_url) return res.status(200).json({ success: true });

  if (op === "upload-session") {
    const rl = await guard({ req, name: "instant-offer-session", perIp: { max: 12, windowSeconds: 600 }, global: { max: 200, windowSeconds: 600 } });
    if (!rl.allowed) return res.status(429).json({ error: "Please try again in a few minutes." });
    return res.status(200).json(security.createSession());
  }

  if (op === "quote" || op === "eligibility") {
    ...
    let market;
    try { market = await marketFor(reference, dial); }
    catch (error) {
      if (op !== 'eligibility') throw error;
      return res.status(200).json({ ok: true, mode: 'personal', reason: 'pricing_unavailable' });
    }
    const offer = priceIt(market, { condition, set });
    if (op === 'eligibility') return res.status(200).json({ ok: true, ...eligibilityFor(offer) });
  ...
  if (op === "search") {
    ...
    let pieces, rows;
    try {
      pieces = await fetchAllPieces();
      rows = await lookupCatalogMarkets(pieces, q,
        keys => rpc("catalog_market_matches", { p_keys: keys, p_days: 60 }));
    } catch (error) {
      console.error('[instant-offer] Search lookup failed:', error.message);
      return res.status(503).json({ error: 'Watch search is temporarily unavailable. Please try again or enter your watch manually.' });
    }
    const results = searchCatalog(rows, pieces, q, 2000);
  ...
  if (op === "sign-upload") {
    ...
    const { path, photoToken } = security.uploadSpec(body);
    ...
    return res.status(200).json({ path, photoToken, uploadUrl: `${SUPABASE_URL}/storage/v1${signed.url}` });
  }
  ...
  if (op === "photo-read") {
    ...
    const authorized = security.authorizePhotos(body);
    const paths = await security.verifyStoredPhotos(authorized.photos);
    const out = await readPhotos(paths, body.reference);
  ...
  if (op === "submit") {
    ...
    const answers = security.validateSubmission(body);
    const authorized = security.authorizePhotos(body, 3);
    const reference = cleanRef(body.reference);
    if (reference.length < 3 && body.quoteMode !== "personal") return res.status(400).json({ error: "Enter the reference number" });
    ...
    const dial = String(body.dial || "").trim().slice(0, 40) || null;
    const { condition, set } = answers;
    const year = Number.parseInt(body.year, 10);
    const paths = authorized.photos.map(p => p.path);
    const submissionHash = crypto.createHash("sha256").update(JSON.stringify({
      reference, dial, email, phone, location, answers, photos: paths,
      option: body.option, quoteMode: body.quoteMode, notes: String(body.notes || "").trim().slice(0, 2000), policyVersion: body.policyVersion,
    })).digest("hex");
    async function existingResult() {
      const r = await sb(`dbh_offers?id=eq.${authorized.session.id}&select=id,submission_hash,offer_response`);
      if (!r.ok) throw new Error("Could not check existing offer");
      const saved = (await r.json())[0];
      if (!saved) return null;
      if (saved.submission_hash !== submissionHash) throw new security.InputError("This session already has a submission. Refresh the page to start another.", 409);
      return { ...saved.offer_response, id: saved.id, duplicate: true };
    }
    const previous = await existingResult();
    if (previous) return res.status(200).json(previous);
    const photos = await security.verifyStoredPhotos(authorized.photos);
    ...
    const [market, photo] = await Promise.all([
      body.quoteMode === 'personal' ? Promise.resolve(null) : marketFor(reference, dial).catch(() => null),
    ...
    const offer = priceIt(market, { condition, set });
    offer.mode = body.quoteMode === 'personal' || !offer.ok ? 'personal' : 'instant';
    if (security.needsPhotoReview(photo)) offer.needs_review = true;
    // A high headline confidence is not a substitute for an agreed valuation.
    if (offer.confidence !== "HIGH") offer.needs_review = true;

    const sellOption = body.option;
    if (sellOption === "consign") {
      offer.needs_review = true;
      offer.amount = offer.low = offer.high = offer.expires_at = null;
    }

    const marketStats = offer.reason === 'invalid_market' ? null : market;
    const row = {
      id: authorized.session.id,
      submission_hash: submissionHash,
      offer_response: offer,
      contact_ok: true,
      authentication_acknowledged: true,
      consented_at: new Date().toISOString(),
      policy_version: security.POLICY_VERSION,
      pricing_version: security.PRICING_VERSION,
      sell_option: sellOption,
      first_name: answers.firstName,
      last_name: answers.lastName,
      has_box: answers.hasBox,
      has_papers: answers.hasPapers,
      worn: answers.worn,
      polished: answers.polished,
      ...
      market_n: marketStats?.n ?? null,
      market_p10: marketStats?.p10 ?? null,
      market_p90: marketStats?.p90 ?? null,
    ...
    try {
      ...
      const subject = sellOption === "consign"
        ...
        : offer.ok
          ...
          : `\u{26A1} Personal quote request: ${what}`;
...
function eligibilityFor(offer) {
  return offer.ok && !offer.needs_review ? { mode: 'instant', reason: null } : { mode: 'personal', reason: offer.reason || 'review_required' };
}

module.exports = { handle, eligibilityFor, priceIt, readPhotos, refCore, MARGIN, WINDOW_DAYS, CONDITIONS, SETS };
