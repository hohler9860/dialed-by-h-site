...
module.exports = async function socialAdmin(body, options = {}) {
    ...
    if (!env.SOCIAL_POSTER_URL || !env.SOCIAL_POSTER_SECRET) {
        if (action === 'status') return { configured: false, reason: 'The website admin is live. Its publishing worker has not been connected yet.', setup: { missing: ['SOCIAL_POSTER_URL', 'SOCIAL_POSTER_SECRET'].filter(key => !env[key]) }, reddit: { status: 'approval_required', connected: false } };
