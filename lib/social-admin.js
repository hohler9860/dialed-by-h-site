...
const ACTIONS = new Set(['status','verify-account','pause','resume','mode','approve','reject','edit','recheck']);
module.exports = async function socialAdmin(body, options = {}) {
    ...
    for (const key of ['id','version','mode','body','scheduledAt']) if (body[key] !== undefined) payload[key] = body[key];
    let response;
    try {
