...
import {workflowAuthorized,beginBatch,commitBatch,failBatch} from './src/external-drafts.js';
...
// Narrow credential: drafts only. It cannot resume publishing, change settings or access account tokens.
export async function workflow(authorization,input,options={}){
  if(!workflowAuthorized(authorization,options.env||process.env))throw error('Unauthorized',401);
  if(!['begin','commit','fail'].includes(input.operation))throw error('Unknown drafting operation.',400);
  if(Buffer.byteLength(JSON.stringify(input))>64000)throw error('Draft batch is too large.',413);
  return withRuntime(options,async ctx=>{
    if(input.operation==='begin'){
      if(!settings(ctx.db).n8nDrafts)return {skip:true,reason:'n8n drafting is disabled.'};
      // Fetch the same anonymous public catalogue a website visitor can read. No database credentials.
      const response=await (options.contextFetcher||fetch)(SITE+'/api/get-inventory',{redirect:'error',signal:AbortSignal.timeout(15000)});
      if(!response.ok)throw error('Public watch catalogue is unavailable.');
      const publicRows=await response.json();if(!Array.isArray(publicRows))throw error('Invalid public catalogue response.');
      const rows=publicRows.filter(p=>p.image).map(p=>({id:p.id,brand:String(p.brand||'').slice(0,100),model:String(p.model||'').slice(0,140),reference:String(p.ref||'').slice(0,100)}));
      return beginBatch(ctx.db,{catalogue:rows,day:input.day});
    }
    return input.operation==='commit'?commitBatch(ctx.db,input):failBatch(ctx.db,input);
  });
}
