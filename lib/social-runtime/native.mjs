...
export async function withRuntime({env=process.env,store=new Store(env),readOnly=false,fetcher=fetch,started=Date.now()},task){
  ...
  try{
    ...
    if(!readOnly){
      for(const media of db.prepare('SELECT * FROM media').all())if(!knownMedia.has(media.id))await store.upload(media,readFileSync(join(dataDir,'media',media.filename)));
      await checkpoint();
    }
    return result;
  }finally{
...
export async function admin(input,options={}){
  ...
  const readOnly=input.action==='status'||(input.action==='workspace'&&['state','link'].includes(input.operation));
