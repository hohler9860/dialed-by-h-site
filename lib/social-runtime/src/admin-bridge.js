...
export function socialStatus(db,x,config) {
  ...
  return {configured:true,serverTime:nowISO(),paused:s.paused,autonomy:s.autonomy,autoGenerate:s.autoGenerate,
    ...
    draftingConfigured:!!config.openaiKey,serviceUrl:config.appUrl,estimatedSpend:monthSpend(db),monthlyBudget:s.monthlyXBudget,
...
export async function bridgeAction(db,x,config,input) {
  switch(input.action) {
    ...
    case 'mode':
      ...
      if(input.mode==='autonomous'&&(!config.openaiKey||!x.account())) throw new Error('Connect X and the drafting API before enabling autonomy.');
      setSetting(db,'autonomy',input.mode);if(input.mode==='autonomous') setSetting(db,'autoGenerate',true);
