...
export function assertNoEmDashes(text) { if(/[\u2014\u2e3a\u2e3b\ufe58]/u.test(text)) throw Object.assign(new Error('Em dashes are not allowed in posts. Rewrite with a comma, colon or period.'),{status:400}); }
...
export function quality(db,post) {
  ...
  try { assertNoEmDashes(body); } catch(e) { errors.push(e.message); }
