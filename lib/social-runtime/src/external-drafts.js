...
export function beginBatch(db,{catalogue=[],day:requestedDay}={}){
  const s=settings(db),today=DateTime.now().setZone(s.timezone).startOf('day'),day=requestedDay??today.plus({days:1}).toISODate();
  if(typeof day!=='string'||!/^\d{4}-\d{2}-\d{2}$/.test(day))throw fail('Use a draft date in YYYY-MM-DD format.');
  const target=DateTime.fromISO(day,{zone:s.timezone});
  if(!target.isValid||target<=today||target>today.plus({days:7}))throw fail('Choose a draft date within the next seven days.');
  ...
  if(db.prepare("SELECT COUNT(*) n FROM spend WHERE kind='n8n_generation' AND created_at>=?").get(today.toUTC().toISO()).n>=2)return {skip:true,reason:'Daily drafting attempt limit reached.'};
