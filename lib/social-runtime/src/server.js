...
import { categories,createPost,approve,quality,seedDrafts,assertNoEmDashes } from './editorial.js';
    ...
    try {
      ...
      if(path==='/api/state' && req.method==='GET') {
        ...
        return json(res,{settings:settings(db),posts,categories,sources:db.prepare('SELECT * FROM sources ORDER BY fetched_at DESC LIMIT 200').all(),
          ...
          media:db.prepare('SELECT m.id,m.alt,m.rights,c.source_url,c.brand,c.model,c.reference FROM media m LEFT JOIN media_catalog c ON c.media_id=m.id').all(),
      ...
      if(path==='/api/settings' && req.method==='PATCH') {
        ...
        if('n8nDrafts' in input){if(typeof input.n8nDrafts!=='boolean')throw fail('Invalid n8n setting.');if(input.n8nDrafts&&!old.n8nWorkflowUrl)throw fail('Install the n8n workflow before enabling daily drafts.');updates.n8nDrafts=input.n8nDrafts;if(input.n8nDrafts)updates.autoGenerate=false;}
        if('autoGenerate' in input) {if(typeof input.autoGenerate!=='boolean') throw fail('Invalid drafting setting.');if(input.autoGenerate&&(input.n8nDrafts??old.n8nDrafts))throw fail('Disable n8n drafting before enabling the native generator.');if(input.autoGenerate&&!config.openaiKey) throw fail('Connect the drafting API first.');updates.autoGenerate=input.autoGenerate;}
        if('autoSiteImages' in input){if(typeof input.autoSiteImages!=='boolean')throw fail('Invalid image setting.');updates.autoSiteImages=input.autoSiteImages;}
      ...
      if(match) {
        ...
        if(req.method==='GET'&&match[2]==='link') {
          throw fail('Links are disabled in posts. Use a reply or DM call to action.');
        }
