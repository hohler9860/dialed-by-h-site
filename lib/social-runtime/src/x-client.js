...
import {postLinks,assertNoEmDashes} from './editorial.js';
...
export class XClient {
  ...
  async publish(post,mediaId) {
    assertNoEmDashes(post.body);
    if(postLinks(post.body).length)throw Object.assign(new Error('Links are disabled in posts. Remove the URL before publishing.'),{status:400});
