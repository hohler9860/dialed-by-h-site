...
test('No-link policy blocks explicit and bare-domain URLs before approval or any X request',async t=>{
 const db=fixture(t);let calls=0;
 const x=new XClient(db,{secret:'a'.repeat(64)},async()=>{calls++;throw Error('Unexpected network');});
 for(const url of ['https://example.com/source','www.example.com','example.com/source']){
  const p=createPost(db,{body:'Ask about sourcing at '+url,category:'Sourcing',scheduledAt:new Date(Date.now()+3600000).toISOString()});
  assert.ok(quality(db,p).errors.some(e=>e.includes('Links are disabled')));
  assert.throws(()=>approve(db,p.id,p.version),/Links are disabled/);
  await assert.rejects(()=>x.publish(p),/Links are disabled/);
 }
 assert.equal(calls,0);
 const plain=createPost(db,{body:'Looking for a particular reference? Send me a DM with the watch you want.',category:'Sourcing'});
 assert.equal(quality(db,plain).errors.some(e=>e.includes('Links are disabled')),false);
});
test('Em dash rule blocks new drafts, legacy approval and X sends before network access',async t=>{
 const db=fixture(t);let calls=0;const x=new XClient(db,{secret:'a'.repeat(64),beforePublish:async()=>{calls++;}},async()=>{calls++;throw Error('Unexpected network');});
 for(const dash of ['\u2014','\u2e3a','\u2e3b','\ufe58']){
  const body='Check the bracelet'+dash+'ask for spare links.';
  assert.throws(()=>createPost(db,{body,category:'Buyer advice',ai:true}),/Em dashes/);
  const p=createPost(db,{body:'Check the bracelet. Ask for spare links.',category:'Buyer advice',scheduledAt:new Date(Date.now()+3600000).toISOString()});
  db.prepare('UPDATE posts SET body=? WHERE id=?').run(body,p.id);p.body=body;
  assert.ok(quality(db,p).errors.some(e=>e.includes('Em dashes')));
  assert.throws(()=>approve(db,p.id,p.version),/Em dashes/);
  await assert.rejects(()=>x.publish(p),/Em dashes/);
 }
 assert.equal(calls,0);
 assert.doesNotThrow(()=>createPost(db,{body:'Pre-owned watches: compare condition, service history and return terms.',category:'Buyer advice'}));
});
