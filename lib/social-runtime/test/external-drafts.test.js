...
test('Superseded runs, disabled automation, two-attempt cap and monthly budget fail closed',t=>{
 ...
 db.prepare('DELETE FROM draft_batches').run();db.prepare('DELETE FROM spend').run();db.prepare("INSERT INTO spend(kind,amount,created_at) VALUES ('n8n_generation',1,?)").run(new Date().toISOString());assert.match(beginBatch(db).reason,/Monthly/);
});
...
test('An explicit future date preserves tomorrow and rejects out-of-range or malformed dates',t=>{const db=setup(t),first=beginBatch(db);commitBatch(db,batch(first));const day=DateTime.now().setZone('America/New_York').plus({days:2}).toISODate();const second=beginBatch(db,{day});assert.equal(second.skip,false);assert.equal(second.day,day);assert.equal(db.prepare('SELECT COUNT(*) n FROM posts WHERE batch_date=?').get(first.day).n,5);for(const day of ['2020-01-01','2100-01-01','2026-99-01','not-a-date'])assert.throws(()=>beginBatch(db,{day}),/date/);});
