...
test('A failed image upload never commits a snapshot referencing absent bytes',async()=>{
 const store=new MemoryStore();store.upload=async()=>{throw Error('storage unavailable');};
 const data=Buffer.concat([Buffer.from([137,80,78,71,13,10,26,10]),Buffer.alloc(40)]);
 await assert.rejects(()=>admin({action:'workspace',operation:'upload',payload:{data:data.toString('base64'),alt:'Watch',rights:'Owned photograph'}},options(store)),/storage unavailable/);
 assert.equal(store.row.state.tables,undefined);assert.equal(store.row.lease_owner,null);
});
