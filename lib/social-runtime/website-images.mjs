...
export function imageCandidate(db){
  return db.prepare("SELECT p.* FROM posts p LEFT JOIN post_image_checks c ON c.post_id=p.id LEFT JOIN editorial_reviews r ON r.post_id=p.id WHERE p.status='draft' AND p.ai_generated=1 AND p.media_id IS NULL AND r.post_id IS NULL AND (c.post_id IS NULL OR c.version!=p.version) ORDER BY p.created_at LIMIT 1").get();
}
