...
async function uploadImage(file) {
  const buf = fs.readFileSync(path.join(DIR, 'celeb_png', file));
...
async function insert(row) {
  ...
  if (row.celebs && row.celebs.length) props.Celebrity = { multi_select: row.celebs.map(name => ({ name })) };
...
// ---------- main ----------
const keep = JSON.parse(fs.readFileSync(path.join(DIR, 'celeb_insert.json')));
const ledgerPath = path.join(DIR, 'celeb_inserted.json');
