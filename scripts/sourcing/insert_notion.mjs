...
function parse(row) {
  ...
  if (brand === 'Richard Mille' && ref && !/^RM/i.test(ref)) ref = 'RM' + ref;
  ...
  const collections = ["Women's"];
  if (model && !gem) {
    if (ICONS.test(model)) collections.push('Classics');
    if (EVERYDAY.test(model) && /steel|oystersteel/i.test(row.title)) collections.push('Everyday Wear');
  }
  ...
  const tags = [...new Set([
    ...
    'womens luxury watches', `ladies ${brand.toLowerCase()}`, `${brand.toLowerCase()} ${model ? model.toLowerCase() : 'watch'} for sale`,
...
async function uploadImage(file) {
  const buf = fs.readFileSync(path.join(DIR, 'wa_png', file));
...
// ---------- main ----------
const keep = JSON.parse(fs.readFileSync(path.join(DIR, 'wa_keep.json')));
const ledgerPath = path.join(DIR, 'wa_inserted.json');
...
const TOO_BIG = /RM\s?27|Crash|Minute Repeater|Sonnerie|Grande? Complication\b|Baguette Diamonds|Sapphire Crystal Case|Grandmaster|Tiffany/i;
const tooBig = [];
